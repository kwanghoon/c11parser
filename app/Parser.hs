...
toProdRule [x] = x ++ " -> "
toProdRule (x:xs) = toProdRule [x] ++ tpr xs
  where
    tpr []     = ""
    tpr (x:xs) = " " ++ x ++ tpr xs
...
init_declarator_list declarator =
  map toProdRule $
    map (nonTerminal "init_declarator_list" [declarator] :)
    [ init_declarator declarator ]
    ...
    map (nonTerminal "init_declarator_list" [declarator] :)
    [ [idl, ",", id]
    | idl <- init_declarator_list declarator,
      id <- init_declarator declarator
    ]
